...
bus.write_byte_data(DEVICE_ADDR, reg["PWR_MGMT_1"], clk_sel)
bus.write_byte_data(DEVICE_ADDR, reg["GYRO_CONFIG"], FS_SEL[250])
bus.write_byte_data(DEVICE_ADDR, reg["ACCEL_CONFIG"], AFS_SEL[16])
...
IMUPoint.__add__ = sum

def imudata(addr):
    data = bus.read_i2c_block_data(DEVICE_ADDR, addr, 14)
    data = [int.from_bytes(data[i:i+2], 'big', signed=True) for i in range(0, len(data), 2)]
    data[0:3] = [i / LSB_SENS[16] for i in data[0:3]]
    data[3] = temp_deg(data[3])
    data[4:8] = [i / LSB_SENS[250] for i in data[4:8]]
    return IMUPoint(*data)

data = []
for i in range(10000):
    data.append(imudata(reg["ACCEL_XOUT_H"]))
    print(i, end="\r")
    time.sleep(0.002)

print("done\t\t")
print("ID:")
print(int(bus.read_byte_data(DEVICE_ADDR, reg["WHO_AM_I"])))
print("Taking mean:")
start = IMUPoint(*([0] * 7))
for i in data:
    start = start + i
mean = IMUPoint(*[i / len(data) for i in start])
mean.az = mean.az - 1
print(mean)
